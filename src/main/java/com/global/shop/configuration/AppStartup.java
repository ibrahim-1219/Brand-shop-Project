...
import com.global.shop.service.ConsumeProductService;
...
public class AppStartup implements CommandLineRunner {
	...
	private final ConsumeProductService consumeProductService;
	...
	public void run(String... args) throws Exception {
		...
		productService.insertAll(Arrays.asList(product1, product2, product3, product4, product5, product6, product7,
		 ...
		 * int quantity = 0;
		 * 
		 * // Convert ProductDto list to Product list
		 * 
		 * List<Product> products = new ArrayList<>();
		 * 
		 * for (ProductDto productDto : productDtos) { Product product = new Product();
		 * product.setId(productDto.getId()); product.setName(productDto.getTitle());
		 * product.setPrice(productDto.getPrice());
		 * product.setDescription(productDto.getDescription()); quantity = quantity +
		 * 10; product.setQuantity(quantity); // Add any additional mappings if needed
		...
		if (customerService.findAll().isEmpty()) {
			...
			// add some users 
			Customer user1=new Customer(); user1.setName("hema");
			...
			user2.addRole(role);
			user2.setCart(cartService.findById(2));
			...
			user3.addRole(role);
			user3.setCart(cartService.findById(3));
			...
			user4.addRole(role);
			user4.setCart(cartService.findById(4));
			...
			user5.addRole(role);
			user5.setCart(cartService.findById(5));
			...
			user6.addRole(role);
			user6.setCart(cartService.findById(6));
			...
			user7.addRole(role);
			user7.setCart(cartService.findById(7));
			...
			user8.addRole(role);
			user8.setCart(cartService.findById(8));
			...
			user9.addRole(role);
			user9.setCart(cartService.findById(9));
			...
			user10.addRole(role);
			user10.setCart(cartService.findById(10));

			customerService
					.insertAll(Arrays.asList(user1, user2, user3, user4, user5, user6, user7, user8, user9, user10));
